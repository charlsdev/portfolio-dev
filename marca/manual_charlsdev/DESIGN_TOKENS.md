# Design Tokens — charlsdev Brand System

Sistema de diseño para el portafolio de Carlos A. Villacreses (charlsdev). Todos los tokens están en formatos consumibles: HEX, CSS variables y Tailwind config.
...
El logo es un **monograma**: la letra **C** (de Carlos / charlsdev) que contiene el símbolo `</>`
de código del logo original. Conserva el símbolo y el color ámbar de la marca anterior;
evoluciona al encapsularlo en la inicial. Todos los archivos están en `/logos`.

### Versiones disponibles
| Archivo | Uso |
|---|---|
| `logo_primary.svg` | Versión principal. Ámbar sobre fondos oscuros. |
| `logo_black.svg` | Mono negro. Fondos claros y documentos impresos. |
| `logo_white.svg` | Mono blanco. Fondos oscuros sin acento de color. |
| `logo_avatar_dark.svg` | Monograma ámbar en cuadrado negro redondeado. Avatar redes. |
| `logo_avatar_amber.svg` | Monograma negro en cuadrado ámbar. Avatar alternativo. |
| `logo_avatar_mono.svg` | Monograma blanco en cuadrado negro. Avatar monocromo. |
| `favicon.svg` + `favicon_16/32/48/64/180.png` | Favicon y app icon multi-tamaño. |

### Reglas de uso
- **Es un monograma / isotipo** (símbolo solo). Si se necesita imagotipo (símbolo + nombre),
  ubicar el wordmark "charlsdev" a la derecha con un gap igual al radio de la C.
- **Tamaño mínimo digital**: 28×28px. Para favicon: 20×20px.
- **Por debajo de 28px**: usar siempre una variante avatar (con fondo) — el contenedor
  preserva la legibilidad del símbolo interno.
- **Área de seguridad**: `x` = 20% del lado del bounding box. Mantener libre alrededor.
- **Cuándo usar cada versión**:
  ...
  - Avatar de plataformas (GitHub, LinkedIn, X) → `logo_avatar_dark.svg`
  - Favicon del sitio → `favicon.svg` (con fallback a los PNG)


### Wordmark (logo + nombre)

Cuando se necesita el nombre visible, usar el **wordmark**: monograma + "charlsdev"
en Space Grotesk Bold (700). Archivos en `/wordmark`.

| Archivo | Uso |
|---|---|
| `wordmark_primary.svg` | Todo ámbar. Fondos oscuros sin más color. |
| `wordmark_dark_bg.svg` | Monograma ámbar + texto papel. **Versión recomendada para fondo oscuro.** |
| `wordmark_black.svg` | Todo negro. Fondos claros, documentos. |
| `wordmark_white.svg` | Todo blanco. Fondos oscuros o de color. |

- El texto va **convertido a trazados** (no depende de la fuente instalada).
- Composición horizontal por defecto; el gap monograma↔texto = radio de la C.
- Para espacios cuadrados, usar el monograma solo (ver sección anterior) o la
  composición vertical documentada en el manual de marca (pág. 05).
- Altura mínima del wordmark: 16px (por debajo, usar solo el monograma).
